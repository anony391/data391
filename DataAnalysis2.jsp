...
if(request.getParameter("Submit") != null) {	//extracting logininfo
    ...
    sqlString += " COUNT(image_id) FROM test_view ";
	...
	int patientInt = Integer.parseInt(patientID);
        String patientSql = "SELECT p.first_name, p.last_name" + 
	" FROM persons p, radiology_record r" +
	" WHERE r.patient_id = p.person_id AND r.patient_id="+patientInt+"";
	try {
		stmt2 = conn.createStatement();
		rset2 = stmt2.executeQuery(patientSql);
	...
	if(rset2 != null && rset2.next()) {
		first = rset2.getString("first_name");
		last = rset2.getString("last_name");
	}
